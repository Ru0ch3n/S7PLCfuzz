...
def init_smart_fuzzing_tree():
    ...
    for nodes in smart_fuzzing_tree.leaves():
        smart_fuzzing_tree.create_node(tag='00', parent=nodes.identifier )
        smart_fuzzing_tree.create_node(tag='01120a101d000200001d000001', parent=nodes.identifier )
        smart_fuzzing_tree.create_node(tag='01120a10020002000082000000', parent=nodes.identifier )
        smart_fuzzing_tree.create_node(tag='01120a10020001000184000008', parent=nodes.identifier)
        smart_fuzzing_tree.create_node(tag='ff120a10020002000082000000', parent=nodes.identifier)
        # 取自抓包中的部分除读写外的特殊功能数据
        smart_fuzzing_tree.create_node(tag='000000000009505f50524f4752414d', parent=nodes.identifier )
        smart_fuzzing_tree.create_node(tag='000000000000fd00000', parent=nodes.identifier )
        smart_fuzzing_tree.create_node(tag='000000000000fd000009505f50524f4752414d', parent=nodes.identifier )
        smart_fuzzing_tree.create_node(tag='ffffffff', parent=nodes.identifier )
        smart_fuzzing_tree.create_node(tag='000000000000fd0002432009505f50524f4752414d', parent=nodes.identifier )
        smart_fuzzing_tree.create_node(tag='000000000000fd0000055f47415242', parent=nodes.identifier )
    print('已生成至8层')
    #8 Data, 项目数或保留
    for nodes in smart_fuzzing_tree.leaves():
        ...
        smart_fuzzing_tree.create_node(tag='ff0900083041303030303141', parent=nodes.identifier)
        smart_fuzzing_tree.create_node(tag='ff09000404240000', parent=nodes.identifier)
        smart_fuzzing_tree.create_node(tag='ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff', parent=nodes.identifier)
...
def load_smart_fuzzing_tree():
    global smart_fuzzing_tree
    with open('E:/Desktop/毕业设计（论文）手册及表格/My_SCADA/2022-05-27-06_26_04init_smart_fuzzing_tree.pickle', 'rb')  as f:
...
def test_proccess(s7comm_data):
    ...
    ss = StreamSocket(s, Raw)    # StreamSocket是Scapy对原生的的包装增强。使用Kernel级别TCP Stack，故不再像前用户态版本一样需要手动配置防火墙规则拦截Kernel堆栈发出的RST数据包，且大大提升测试效率
    try:
        ss.sr(cotp_cr_packet, verbose=0)
        ss.sr(s7comm_setup_packet, verbose=0)
        while local.counter < 10:
            fuzz_packet = Packet(str2byte(get_s7_smart_fuzz_data(s7comm_data[local.counter])))
...
def exstract_data_from_tags(tags:list):
    ...
    if '%PL0' in tags:
        tags[tags.index('%PL0')] =  hex(int((len(tags[-1]) + len(tags[-2])) / 2))[2:].zfill(4)
    if '%end' in tags:
        tags[tags.index('%end')] = ''
    return "".join(tags)
...
if __name__ == '__main__':
    ...
    for i in range(1000):
        ...
        p1 = Process(target=test_proccess,args=(exstract_data_from_tags_list(all_tree_paths_tags[all_tags_index:all_tags_index+10]), ))
