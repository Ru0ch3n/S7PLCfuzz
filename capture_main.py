...
SNAP7_DLL_PATH = (
    "C:\ProgramData\Miniconda3\envs\My_SCADA\Lib\site-packages\snap7\lib\snap7.dll"
)
# 定义被测试的PLC信息，包括ip、型号
...
def capture(func):
    def wrapper(self):
        ...
        # 保存pcap
        wrpcap(
            filename=OUTPUT_DIR + func.__name__ + get_time_str() + ".pcap",
            pkt=sniff_work.results,
        )
        # 序列化PacketList对象
        with open(
            OUTPUT_DIR + func.__name__ + get_time_str() + ".pickle", "wb"
        ) as f:  # 打开文件
...
def load_pickle():
    ...
    pickles_dict: Dict[
        str, scapy.plist.PacketList
    ] = {}  # 返回类型为字典，键值对为<序列化文件名>:<PacketList>
...
class MyTest:
    ...
    def test_read_multi_vars(self):
        ...
        for di in data_items:
            ...
            pBuffer = ctypes.cast(
                ctypes.pointer(dataBuffer), ctypes.POINTER(ctypes.c_uint8)
            )
            di.pData = pBuffer
    ...
    def test_read_area(self):
        ...
        assert all(
            data[i] == bytearray(res)[i] for i in range(len(data))
        ), "They aren't always equal"
    ...
    def test_write_area(self):
        ...
        try:
            ...
            res = self.client.read_area(area, dbnumber, start, 1)
            assert all(
                timer[i] == bytearray(res)[i] for i in range(len(timer))
            ), "They aren't always equal"
        except:
            print("CLI : function refused by CPU (Unknown error)")
    ...
    def test_as_ct_read(self):
        ...
        try:
            ...
            self.client.wait_as_completion(500)
            assert all(
                expected[i] == bytearray(buffer)[i] for i in range(len(expected))
            ), "They aren't always equal"
        except Snap7Exception:
            print("CLI : function refused by CPU (Unknown error)")
    ...
    def test_as_db_fill(self):
        ...
        try:
            ...
            assert all(
                expected[i] == bytearray(result)[i] for i in range(len(expected))
            ), "They aren't always equal"
        except Snap7Exception:
    ...
    def test_as_db_read(self):
        ...
        self.client.wait_as_completion(500)
        assert all(
            expected[i] == bytearray(data)[i] for i in range(len(data))
        ), "They aren't always equal"
    ...
    def test_as_db_write(self):
        ...
        #     print(result[i], end=" ")
        assert all(
            result[i] == bytearray(data)[i] for i in range(len(data))
        ), "They aren't always equal"
