...
def COTP_CC_AND_S7COMM_REQ(SYNACK):
    ...
    COTP_CR = (
        ip
        / TCP(sport=sport, dport=dport, flags="PA", seq=SYNACK.ack, ack=SYNACK.seq + 1)
    ...
    S7COMM_SETUP_REQUEST = (
        ip
        / TCP(
            sport=sport,
            dport=dport,
            flags="PA",
            seq=COTP_CC[0][1][1].ack,
            ack=COTP_CC[0][1][1].seq + len(COTP_CC[0][1][1].load),
        )
        / str2byte(set_comm)
    )
    S7COMM_SETUP_CONFORM = sr(
        S7COMM_SETUP_REQUEST,
        multi=True,
        timeout=3,
        iface=iface_getter(S7COMM_SETUP_REQUEST),
    )
    # 发送 TCP ack
    comm_ack = TCP(
        sport=sport,
...
def fuzz(comm_ack):
    ...
    fuzz_package = (
        ...
        / TCP(
            ...
            dport=dport,
            flags="PA",
            seq=comm_ack[0][1][1].ack,
            ack=comm_ack[0][1][1].seq + len(comm_ack[0][1][1].load),
        )
        / fuzz_pkt
    )
