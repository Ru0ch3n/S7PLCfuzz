...
def test_proccess(s7comm_data):
	...
	ss = StreamSocket(s, Raw)    # StreamSocket是Scapy对原生Socket的包装增强。使用Kernel级别TCP Stack，故不再像前用户态版本一样需要手动配置防火墙规则拦截Kernel堆栈发出的RST数据包，且大大提升测试效率
	try:
...
def exstract_data_from_tags(tags:list):
	...
	if '%PL0' in tags:
		tags[tags.index('%PL0')] =  str(int((len(tags[-1]) + len(tags[-2])) / 2)).zfill(4)
		print(tags[4])
	if '%end' in tags:
		tags[tags.index('%end')] = ''
	return "".join(tags)
...
if __name__ == '__main__':
	...
	round = 0
	for i in range(1000):
		p1 = Process(target=test_proccess,args=(exstract_data_from_tags(all_tree_paths_tags[all_tags_index]), ))
